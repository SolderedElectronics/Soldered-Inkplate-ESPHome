...
void Inkplate4::dump_config() {
  ESP_LOGCONFIG(TAG, "Inkplate 4 %dx%d, dark_phases=%d, partial_phases=%d, grayscale_phases=%d",
  ...
  switch (this->trf_sub_) {
    ...
    case TRF_GRAYSCALE_SEND: {
      ...
      for (int i = 0; i < this->height_; i++) {
        ...
        this->dma_line_buf_[this->width_ / 4]     = (uint8_t)(this->glut2_[this->trf_k_ * 256 + *(--dp)] | this->glut_[this->trf_k_ * 256 + *(--dp)]);
        this->dma_line_buf_[this->width_ / 4 + 1] = (uint8_t)(this->glut2_[this->trf_k_ * 256 + *(--dp)] | this->glut_[this->trf_k_ * 256 + *(--dp)]);
        send_line_i2s_();
        vscan_end_();
      }
      esp_rom_delay_us(230);
